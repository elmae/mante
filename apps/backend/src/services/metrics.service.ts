import { DataSource, Repository, Between, FindOptionsWhere, In, Not, IsNull } from 'typeorm';
...
import {
  ...
  CategoryMetrics,
  MetricsSummary
} from '../domain/dtos/metrics-filter.dto';
...
export class MetricsService {
  ...
  private buildWhereClause(filter?: MetricsFilterDTO): FindOptionsWhere<Ticket> {
    ...
    if (filter.startDate && filter.endDate) {
      whereClause.created_at = Between(filter.startDate, filter.endDate);
    }
    ...
    if (filter.tags?.length) {
      whereClause.tags = In(filter.tags);
    }
  ...
  async getMetricsSummary(options?: MetricsQueryOptions): Promise<MetricsSummary> {
    const [timeMetrics, ticketMetrics] = await Promise.all([
      this.calculateTimeMetrics(options),
      this.getTicketMetrics(options)
    ]);

    let additionalMetrics = {};

    if (options?.aggregation?.includeTechnicianMetrics) {
      const technicianMetrics = await this.getTechnicianMetrics(options);
      additionalMetrics = { ...additionalMetrics, technicianMetrics };
    }

    if (options?.aggregation?.includeATMMetrics) {
      const atmMetrics = await this.getATMMetrics(options);
      additionalMetrics = { ...additionalMetrics, atmMetrics };
    }

    const categoryMetrics = await this.getCategoryMetrics(options);
  ...
  async calculateTimeMetrics(options?: MetricsQueryOptions): Promise<TimeMetrics> {
  ...
  async getTicketMetrics(options?: MetricsQueryOptions): Promise<TicketMetrics> {
    const whereClause = this.buildWhereClause(options?.filter);
  ...
  async getHistoricalMetrics(options?: MetricsQueryOptions): Promise<HistoricalMetrics[]> {
    const filter = options?.filter ?? {};
    const timeUnit = options?.aggregation?.timeUnit ?? 'day';
  ...
  async getTechnicianMetrics(options?: MetricsQueryOptions): Promise<TechnicianMetrics[]> {
    const whereClause = this.buildWhereClause(options?.filter);
  ...
  async getATMMetrics(options?: MetricsQueryOptions): Promise<ATMMetrics[]> {
    const whereClause = this.buildWhereClause(options?.filter);
  ...
  async getCategoryMetrics(options?: MetricsQueryOptions): Promise<CategoryMetrics[]> {
    const whereClause = this.buildWhereClause(options?.filter);
